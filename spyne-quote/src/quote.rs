    ...
    for tok in stream {
        match tok {
            proc_macro::TokenTree::Ident(i) => {
                out.push(TokenTree::Ident(i.to_string(), Span::default()));
            }
            proc_macro::TokenTree::Punct(p) => {
                match p.spacing() {
                    proc_macro::Spacing::Alone => out.push(TokenTree::Punct(p.as_char(), Spacing::Alone, Span::default())),
                    proc_macro::Spacing::Joint => out.push(TokenTree::Punct(p.as_char(), Spacing::Joint, Span::default())),
                }
            }
            proc_macro::TokenTree::Literal(l) => {
                out.push(TokenTree::Literal(l.to_string(), Span::default()));
            }
            proc_macro::TokenTree::Group(g) => {
                match g.delimiter() {
                    proc_macro::Delimiter::Parenthesis => out.push(TokenTree::Group(Delimiter::Parenthesis, from_stream(g.stream()), Span::default())),
                    proc_macro::Delimiter::Brace => out.push(TokenTree::Group(Delimiter::Brace, from_stream(g.stream()), Span::default())),
                    proc_macro::Delimiter::Bracket => out.push(TokenTree::Group(Delimiter::Bracket, from_stream(g.stream()), Span::default())),
                    proc_macro::Delimiter::None => out.push(TokenTree::Group(Delimiter::None, from_stream(g.stream()), Span::default())),
                }
            }
        }
    }
...
pub fn to_stream(out: Vec<TokenTree>) -> TokenStream {
    ...
    for tok in out {
        match tok {
            TokenTree::Ident(i, s) => stream_vec.push(proc_macro::TokenTree::Ident(Ident::new(&i, proc_macro::Span::call_site()))),
            ...
            TokenTree::Literal(l, s) => stream_vec.push(proc_macro::TokenTree::Literal(l.parse().unwrap())),
            TokenTree::Group(d, t, s) => {
                match d {
